...
Every run is written to its own ``output/HH-MM-SS`` folder.
...
TRANSPARENT_ASSETS = {"character", "text", "logo"}
...
LAYOUTS = (
    ...
    Layout(
        ...
        shade_center=0.34,
        shade_width=0.28,
        characters=(CharacterPlacement("all", 0.64735, 927, -32),),
        text_width=869,
        text_x=61,
        text_y=65,
        branding=(("logo", 172, 410, 300),),
    ),
    Layout(
        size=(640, 360),
        ...
        shade_center=0.22,
        shade_width=0.30,
        characters=(CharacterPlacement("all", 0.38487, 195, 41),),
        text_width=289,
        text_x=17,
        text_y=82,
        branding=(("logo", 172, 76, 167),),
    ),
)
...
def create_run_output_dir(output_root: Path) -> Path:
    """Create an HH-MM-SS folder, adding a suffix for same-second runs."""
    ...
    while candidate.exists():
        candidate = output_root / f"{timestamp}_{counter:02d}"
        counter += 1
    candidate.mkdir()
    return candidate
...
def make_banners(
    ...
    run_id = unique_run_id(run_output_dir)
    created: list[Path] = []
    ...
    for layout in LAYOUTS:
        ...
        created.extend(
            save_banner(banner, run_output_dir, layout.label, run_id, save_jpeg)
        )
