...
class MemoryOutput:
    def __init__(self, preset, device, external_key=False, single_port=False):
...
class AlphaTests(unittest.TestCase):
    ...
    def test_runtime_no_scanning_and_transparent_clear(self):
        with tempfile.TemporaryDirectory(dir=Path(__file__).parent) as directory:
            ...
            with patch.object(core, 'DeckLinkLiveOutput', MemoryOutput), patch.object(
                ...
                for bad in ({'preset': 'HD 1080p25'}, {'output_name': list(core.DECKLINK_OUTPUTS)[2]},
                            {'keyer_confirmed': False}):
                ...
                self.assertEqual(runtime.live_output.snapshot_frame().getchannel('A').getextrema(), (255, 255))
    ...
    def test_1080i50_software_pipeline_preserves_alpha(self):
        ...
        self.assertIn('profile=one-sub-device-full', description)
