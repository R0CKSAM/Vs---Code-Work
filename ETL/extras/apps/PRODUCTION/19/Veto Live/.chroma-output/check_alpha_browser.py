    ...
    try:
        with patch.object(core, 'DeckLinkLiveOutput', MemoryOutput), sync_playwright() as p:
            ...
            for width, height in ((1440, 1000), (390, 844)):
                ...
                assert page.locator('#clearMode option[value="chroma-green"], #clearMode option[value="chroma-blue"]').count() == 0
                ...
                page.locator('#liveButton').click()
                page.locator('#clearMode').select_option('chroma-magenta')
                expect(page.locator('#livePreset')).to_have_value('HD 1080i50')
                expect(page.locator('#livePreset')).to_be_disabled()
                expect(page.locator('#outputHardwareStatus')).to_contain_text('SDI 1 | Magenta #FF00FF')
                expect(page.locator('#keyerConfirmationText')).to_contain_text('magenta / pink')
                expect(page.locator('#startLive')).to_be_disabled()
                page.locator('#receiverConfirmed').check()
                page.locator('#keyerConfirmed').check()
                page.screenshot(path=str(Path(__file__).with_name(f'magenta-ports-{width}.png')))
                page.locator('#startLive').click()
                expect(page.locator('#liveStatus')).to_have_text('Magenta / Output running')
                assert runtime.live_output.single_port
                for key in ('t5', 't11'):
                    page.evaluate(f"switchTemplate('{key}')")
                    page.wait_for_function('previewReady')
                    expect(page.locator('#takeLive')).to_be_enabled(timeout=15000)
                    page.locator('#takeLive').click()
                    expect(page.locator('#liveStatus')).to_have_text('On Air')
                    page.wait_for_function('!liveCommandBusy && latestLive.program_name === boot.template_names[template] && programReady && displayedProgramRevision === latestLive.program_revision')
                    assert runtime.live_output.snapshot_frame().getpixel((0, 0)) == (255, 0, 255)
                    page.screenshot(path=str(Path(__file__).with_name(f'magenta-{key}-{width}.png')))
                page.locator('#clearLive').click()
                expect(page.locator('#liveStatus')).to_have_text('Magenta / Output running')
                assert runtime.live_output.snapshot_frame().getextrema() == ((255, 255), (0, 0), (255, 255))
                page.locator('#liveButton').click()
                expect(page.locator('#liveStatus')).to_have_text('Output stopped')
                page.locator('#liveButton').click()
                ...
                print(json.dumps({'viewport': width, 'alpha_and_magenta_start_push_clear_stop': 'PASS', 'scan_requests': 0, 'browser_errors': errors}))
                page.close()
            browser.close()
    finally:
