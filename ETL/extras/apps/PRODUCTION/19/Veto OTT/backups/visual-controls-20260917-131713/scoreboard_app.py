...
DECKLINK_OUTPUTS = {
    ...
    "DeckLink output 3 (device 2)": 2,
    "DeckLink output 4 (device 3)": 3,
}
DECKLINK_KEY_PAIRS = {
    "DeckLink output 1 (device 0)": "SDI 1 key / SDI 2 fill",
    "DeckLink output 2 (device 1)": "SDI 3 key / SDI 4 fill",
}
...
def draw_text(draw, xy, text, font, fill, anchor=None):
    ...
    if isinstance(font, StyledTextFont):
        dx_pct, dy_pct = text_position_offset(font.cfg, font.role)
...
def prepare_broadcast_frame(image: Image.Image, preset_name: str, preserve_alpha: bool = False) -> Image.Image:
    ...
    frame = Image.new(mode, (width, height), (0, 0, 0, 0) if preserve_alpha else (0, 0, 0))
...
def build_decklink_pipeline(preset_name: str, device_number: int, external_key: bool = False, single_port: bool = False) -> str:
    ...
    if external_key and (device_number not in (0, 1) or preset_name != 'HD 1080i50'):
    ...
    output_caps = (
        f"video/x-raw,format={working_format},width={width},height={height},"
    ...
    stages.append(
        ...
        f"{keyer_settings} sync=true"
    )
    return " ! ".join(stages)


def load_gstreamer():
    """Load the bundled SDI plugins without scanning unrelated codecs."""
    try:
        import gstreamer_libs
    ...
    blackmagic = next(
        (
            path for path in (
                Path(r"C:\Program Files\Blackmagic Design\Desktop Video"),
                Path(r"C:\Program Files\Blackmagic Design\Blackmagic Desktop Video"),
            )
            if path.is_dir()
        ),
        Path(r"C:\Program Files\Blackmagic Design\Desktop Video"),
    ...
    sdi_plugins = []
    if os.name == "nt":
        import gstreamer_plugins
        base_plugins = Path(gstreamer_libs.__file__).parent / 'lib' / 'gstreamer-1.0'
        extra_plugins = Path(gstreamer_plugins.__file__).parent / 'lib' / 'gstreamer-1.0'
        sdi_plugins = [base_plugins / name for name in (
            'gstcoreelements.dll', 'gstapp.dll', 'gstvideoconvertscale.dll')]
        sdi_plugins += [extra_plugins / name for name in ('gstinterlace.dll', 'gstdecklink.dll')]
        # Configure before importing Gst: its bindings can initialize the registry.
        environment.update({
            'GST_PLUGIN_PATH_1_0': '', 'GST_PLUGIN_SYSTEM_PATH_1_0': '',
            'GST_REGISTRY_FORK': 'no', 'GST_REGISTRY_UPDATE': 'no',
            'GST_REGISTRY_1_0': str(Path(tempfile.gettempdir()) / 'veto-scoreboard-sdi-registry.bin'),
        })
    os.environ.update(environment)
    if os.name == "nt" and hasattr(os, "add_dll_directory"):
        for directory in str(dll_paths).split(os.pathsep):
    ...
    Gst.init(None)
    for plugin_path in sdi_plugins:
        if not Gst.Plugin.load_file(str(plugin_path)):
            raise RuntimeError(f"Could not load SDI plugin: {plugin_path.name}")
    missing = [
...
class DeckLinkLiveOutput:
    ...
    def __init__(self, preset_name: str, device_number: int, external_key: bool = False, single_port: bool = False):
        ...
        try:
            self.pipeline = self.Gst.parse_launch(
                build_decklink_pipeline(preset_name, self.device_number, external_key, single_port)
            )
    ...
    def _set_frame(self, image: Image.Image) -> None:
        frame = prepare_broadcast_frame(image, self.preset_name, preserve_alpha=True)
        with self._frame_lock:
            self._frame_data = frame.tobytes("raw", "BGRA")
...
def render_t5(cfg: Dict) -> Image.Image:
    ...
    labels = (("AGE", 612), ("RANKING", 756), ("W/L", 913))
    ...
    if transparent:
        opacity = clamp_number(cfg.get('overlay_opacity_pct'), 0, 100, 100) / 100
        img.putalpha(img.getchannel('A').point(lambda value: round(value * opacity)))
    return img
...
# Wide six-stat player lower-third based on the supplied broadcast reference.
T14_SIZES = BROADCAST_SIZES
T14_ART_SIZE = (1900, 480)

DEF_T14 = {
    "template":"t14", "canvas_size":"HD  (1920x1080)",
    "first_name":"SUMIT", "last_name":"NAGAL", "country":"India",
    "age":"29", "singles_wl":"8/6", "total_wl":"8/6",
    "ties_played":"9", "first_year_played":"2016",
    "favourite_hand":"Right (Double Handed\nBackhand)",
    "player_path":"", "player_size_pct":100,
    "player_offset_x_pct":0, "player_offset_y_pct":0,
    ...
    "accent_green":[15,209,58], "panel_white":[247,247,244],
    "stats_green":[0,126,65], "label_color":[30,32,31],
    "separator_color":[116,164,143], "name_text_color":[255,255,255],
    "country_text_color":[0,126,65], "bar_color":None,
    "text_styles":{"player_names":{"case":"UPPERCASE"}, "stat_labels":{"case":"UPPERCASE"}},
    "rows":[],
}

TEXT_STYLE_TARGETS['t14'] = [
    ('all', 'All text'), ('player_names', 'Player name'),
    ('country', 'Country'), ('stat_labels', 'Stat labels'),
    ('stat_values', 'Stat values'), ('favourite_hand', 'Favourite hand'),
]


def render_t14(cfg: Dict) -> Image.Image:
    """Render Player Band V2 as an editable fill/key lower-third."""
    W, H = T14_SIZES.get(cfg.get('canvas_size', 'HD  (1920x1080)'), (1920, 1080))
    render_scale = max(1, int(cfg.get('_render_scale', 1)))
    W, H = W * render_scale, H * render_scale
    transparent = bool(cfg.get('transparent_background', True))
    background = normalize_rgb(cfg.get('background_color'), (255, 255, 255))
    img = Image.new('RGBA', (W, H), (0, 0, 0, 0) if transparent else (*background, 255))

    art_w, art_h = T14_ART_SIZE
    art = Image.new('RGBA', T14_ART_SIZE, (0, 0, 0, 0))
    draw = ImageDraw.Draw(art, 'RGBA')
    green = normalize_rgb(cfg.get('band_green'), (0, 115, 61))
    dark_green = normalize_rgb(cfg.get('band_green_dark'), (0, 88, 48))
    bright = normalize_rgb(cfg.get('accent_green'), (15, 209, 58))
    white = normalize_rgb(cfg.get('panel_white'), (247, 247, 244))
    stat_green = normalize_rgb(cfg.get('stats_green'), (0, 126, 65))
    label_color = normalize_rgb(cfg.get('label_color'), (30, 32, 31))
    separator = normalize_rgb(cfg.get('separator_color'), (116, 164, 143))
    ...
    def style(role, fallback_color, fallback_size, variant='regular'):
        ...
        return color, text_font(cfg, role, size, variant)

    first = apply_text_case(cfg, 'player_names', str(cfg.get('first_name', 'SUMIT')))
    last = apply_text_case(cfg, 'player_names', str(cfg.get('last_name', 'NAGAL')))
    first_color, first_font = style('player_names', name_color, 48, 'italic')
    last_color, last_font = style('player_names', name_color, 73, 'bold_italic')
    first_font = fit_font(draw, first, 360, getattr(first_font, 'size', 48), 24,
                          text_font_factory(cfg, 'player_names', 'italic'))
    last_font = fit_font(draw, last, 385, getattr(last_font, 'size', 73), 30,
                         text_font_factory(cfg, 'player_names', 'bold_italic'))
    ...
    country_font = fit_font(draw, country, 205, getattr(country_font, 'size', 47), 22,
                            text_font_factory(cfg, 'country', 'bold'))
    cw, ch = text_bbox(draw, country, country_font)
    draw_text(draw, (530-cw//2, 368-ch//2), country, country_font, country_color)
    ...
    hand_col, hand_font = style('favourite_hand', stat_green, 27, 'bold')
    top_stats = (
        ('AGE', str(cfg.get('age','29'))),
        ('SINGLES W/L', str(cfg.get('singles_wl','8/6'))),
        ('TOTAL W/L', str(cfg.get('total_wl','8/6'))),
    )
    bottom_stats = (
        ('TIES PLAYED', str(cfg.get('ties_played','9'))),
        ('FIRST YEAR PLAYED', str(cfg.get('first_year_played','2016'))),
        ('FAVOURITE HAND', str(cfg.get('favourite_hand','Right (Double Handed\nBackhand)'))),
    )

    def centered_fit(text, cx, y, max_width, font, role, variant, minimum, color):
        text = apply_text_case(cfg, role, text)
        fitted = fit_font(draw, text, max_width, getattr(font, 'size', minimum), minimum,
                          text_font_factory(cfg, role, variant))
        tw, _ = text_bbox(draw, text, fitted)
        draw_text(draw, (round(cx-tw/2), y), text, fitted, color)

    for index, (label, value) in enumerate(top_stats):
        cx = stats_left + col_w*(index+.5)
        centered_fit(label, cx, top_y+2, int(col_w-28), label_font, 'stat_labels', 'bold', 15, label_col)
        centered_fit(value, cx, top_y+43, int(col_w-34), value_font, 'stat_values', 'bold', 28, value_col)
    for index, (label, value) in enumerate(bottom_stats):
        cx = stats_left + col_w*(index+.5)
        centered_fit(label, cx, split_y+13, int(col_w-28), label_font, 'stat_labels', 'bold', 14, label_col)
        if index < 2:
    ...
    player_x = 238 + art_w*clamp_number(cfg.get('player_offset_x_pct'), -100, 100, 0)/100
    feet_y = 430 + 1080*clamp_number(cfg.get('player_offset_y_pct'), -100, 100, 0)/100
    if portrait is not None and portrait.height:
        ...
        pd = ImageDraw.Draw(placeholder, 'RGBA')
        pd.ellipse((73,16,147,90), fill=(238,241,238,255), outline=(*separator,255), width=3)
        pd.rounded_rectangle((33,84,187,418), radius=40, fill=(238,241,238,255), outline=(*separator,255), width=3)
        placeholder = placeholder.resize((max(1,round(220*portrait_h/420)),portrait_h),Image.LANCZOS)
        art.alpha_composite(placeholder,(round(player_x-placeholder.width/2),round(feet_y-portrait_h)))

    base_scale = min(W/1920, H/1080)
    scale = base_scale * clamp_number(cfg.get('band_scale_pct'), 25, 200, 100)/100
    scaled = art.resize((max(1,round(art_w*scale)),max(1,round(art_h*scale))),Image.LANCZOS)
    center_x = W*clamp_number(cfg.get('band_x_pct'), -50, 150, 50)/100
    center_y = H*clamp_number(cfg.get('band_y_pct'), -50, 150, 74)/100
    img.alpha_composite(scaled,(round(center_x-scaled.width/2),round(center_y-scaled.height/2)))
    if transparent:
        opacity = clamp_number(cfg.get('overlay_opacity_pct'),0,100,100)/100
        img.putalpha(img.getchannel('A').point(lambda value: round(value*opacity)))
    return img
...
WEB_TEMPLATE_NAMES = TEMPLATE_NAMES + ["Player Band", "Players Stats", "Qualifier Rounds", "Custom Upload", "Head2Head", "Player Band V2"]
WEB_TEMPLATE_KEYS = TEMPLATE_KEYS + ("t5", "t6", "t7", "t8", "t9", "t14")
RENDERERS = {
    "t1": render_t1,
    "t2": render_t2,
    "t3": render_t3,
...
def normalise_project_configs(saved: Any) -> Dict[str, Dict]:
    ...
    for key, default in DEFAULT_CONFIGS.items():
        ...
        if key == "t14":
            for field in (
                'first_name','last_name','country','age','singles_wl','total_wl',
                'ties_played','first_year_played','favourite_hand',
            ):
            ...
            numeric_fields = {
                ...
                'band_scale_pct':(25,200),
            }
            for field,(minimum,maximum) in numeric_fields.items():
                config[field] = clamp_number(config.get(field),minimum,maximum,default[field])
            for color_key in (
