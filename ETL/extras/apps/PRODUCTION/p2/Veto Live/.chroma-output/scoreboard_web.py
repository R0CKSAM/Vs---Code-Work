...
class ScoreboardWebRuntime:
    ...
    def output_capabilities(self):
        # Retain the endpoint for open dashboards, using only configured outputs.
        return {
            'devices': [dict(name=name, number=number, model='Configured DeckLink output',
                             modes=list(self.core.VIDEO_EXPORT_PRESETS))
                        for name, number in self.core.DECKLINK_OUTPUTS.items()],
            'source': 'configured', 'switcher_detected': False,
        }
    ...
    def bootstrap(self) -> Dict[str, Any]:
        return {
            ...
            "decklink_key_pairs": self.core.DECKLINK_KEY_PAIRS,
            "canvas_sizes": {
    ...
    def render(
        ...
        if output_mode == 'external-key':
            if template not in ('t5', 't11'):
                raise ValueError('Fill/key overlays support Player Band and Coming Next.')
            config['transparent_background'] = True
        if output_mode in CHROMA_COLORS and template in ('t5', 't11'):
            config['transparent_background'] = True
    ...
    def start_live(
        ...
        if clear_mode.startswith('chroma-') and keyer_confirmed is not True:
        ...
        if clear_mode == 'chroma-magenta':
            if preset != 'HD 1080i50':
                raise ValueError('Magenta overlays require HD 1080i50.')
            if template not in ('t5', 't11'):
                raise ValueError('Magenta overlays support Player Band and Coming Next.')
        if clear_mode == 'external-key':
            if keyer_confirmed is not True:
                raise ValueError('Confirm the paired SDI fill/key connections first.')
        ...
        with self.lock:
            ...
            image, _ = self.render(
                ...
                output_mode=clear_mode, output_preset=preset,
            )
            if (
            ...
            if previous is not None:
                ...
                self.live_output_name = None
                self.live_owner_id = None
            output_options = {'external_key': clear_mode == 'external-key'}
            if clear_mode == 'chroma-magenta':
                output_options['single_port'] = True
            output = self.core.DeckLinkLiveOutput(preset, device_number, **output_options)
    ...
    def show_live(self, template, config, client_id, remote_address, expected_revision=None, name=''):
        with self.lock:
            ...
            if getattr(self, 'live_clear_mode', 'black') == 'chroma-magenta' and template not in ('t5', 't11'):
                raise ValueError('Magenta overlays support Player Band and Coming Next.')
    ...
    def _clear_frame(self, size, mode=None):
        ...
        if mode == 'external-key':
            return self.core.Image.new('RGBA',size,(0,0,0,0))
    ...
    def program_preview(self):
        ...
        if image.mode == 'RGBA':
            ...
            draw = self.core.ImageDraw.Draw(background)
            for y in range(0,image.height,16):
                for x in range(0,image.width,16):
                    if (x//16 + y//16) % 2:
                        draw.rectangle((x,y,x+15,y+15),fill=(72,72,72))
            background.paste(image,(0,0),image.getchannel('A'))
            image = background
        data = io.BytesIO()
