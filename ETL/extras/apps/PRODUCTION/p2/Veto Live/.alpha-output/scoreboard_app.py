...
def build_decklink_pipeline(preset_name: str, device_number: int, external_key: bool = False) -> str:
    ...
    keyer_settings = ('duplex-mode=full video-format=8bit-bgra keyer-mode=external keyer-level=255'
...
class DeckLinkLiveOutput:
    ...
    def __init__(self, preset_name: str, device_number: int, external_key: bool = False):
        ...
        try:
            self.pipeline = self.Gst.parse_launch(
                build_decklink_pipeline(preset_name, self.device_number, external_key)
            )
