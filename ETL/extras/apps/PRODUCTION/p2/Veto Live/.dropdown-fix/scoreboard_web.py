...
class ScoreboardWebRuntime:
    ...
    def output_capabilities(self):
        ...
        with self._capability_lock:
            if self._capabilities and self._capabilities.get('devices') and time.monotonic()-self._capability_time < 30:
            ...
            try:
                ...
                # --target installs are not on the child interpreter's default path.
                package_dir = self.app_dir / 'python_packages'
                if package_dir.is_dir():
                    probe_env['PYTHONPATH'] = str(package_dir)
                ...
                if result.returncode:
                    ...
                    raise ValueError(detail[-1] if detail else f'Driver probe exited with code {result.returncode}.')
