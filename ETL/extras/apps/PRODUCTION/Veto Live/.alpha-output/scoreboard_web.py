...
class ScoreboardWebRuntime:
    ...
    def start_live(
        ...
        if clear_mode not in ('black','chroma-green','chroma-blue','external-key'):
        ...
        with self.lock:
            ...
            output = self.core.DeckLinkLiveOutput(
                preset, device_number, external_key=(clear_mode == 'external-key')
            )
    ...
    def show_live(self, template, config, client_id, remote_address, expected_revision=None, name=''):
        with self.lock:
            ...
            image, config = self.render(template, config, update_live=False,
                output_mode=getattr(self,'live_clear_mode','black'))
