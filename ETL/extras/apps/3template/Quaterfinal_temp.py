...
class ThumbnailGenerator(QMainWindow):
    ...
    def _commit_history(self):
        if self._restoring or self._history_busy or not hasattr(self, "_history_state"):
            return
        try:
            current = json.dumps(self._capture_state())
        except (RuntimeError, TypeError, ValueError, AttributeError):
            # A timer tick can land while Qt is removing/replacing graphics
            # items.  Defer the snapshot until the scene is stable instead of
            # letting an exception escape a Qt slot (which can abort PyQt).
            return
    ...
    def _poll_history(self):
        try:
            if (not self._history_busy and
                    QApplication.mouseButtons() == Qt.MouseButton.NoButton):
                if self.text_edit.hasFocus() and self.text_edit.isModified():
                    return
                self._commit_history()
        except Exception as exc:
            # Exceptions escaping a PyQt timer callback may close the entire
            # application. History polling is best-effort, so report and retry.
            if hasattr(self, "statusBar"):
                self.statusBar().showMessage(f"History snapshot deferred: {exc}", 3000)

    def _travel_history(self, backwards):
        if self._history_busy or self._restoring:
            return
        self._commit_history()
        ...
        try:
            ...
            self.statusBar().showMessage(f"Undo/redo is temporarily unavailable: {exc}", 5000)
            return
        state = source[-1]
        ...
        try:
            ...
            # Never open a nested modal dialog from history restoration. It can
            # process another undo/redo event while the scene is half rebuilt.
            try:
                self._apply_state(json.loads(current))
            except Exception as recovery_exc:
                self.statusBar().showMessage(
                    f"History restore failed safely: {exc}; recovery: {recovery_exc}", 8000
                )
            else:
                self.statusBar().showMessage(f"Could not restore this change: {exc}", 6000)
            return
        finally:
            self._history_busy = False
            if hasattr(self, "_history_timer"):
    ...
    def _build_sidebar(self):
        ...
        tabs = QTabWidget()
        # Keep references to the sidebar tabs/scroll areas so a canvas click can
        # jump straight to the controls for the selected design element.
        self.sidebar_tabs = tabs
        self.sidebar_tab_indices = {}
        self.sidebar_tab_scrolls = {}
        ...
        for page, title in ((edit_tab, "Text"), (assets_tab, "Images"),
            ...
            self.sidebar_tab_indices[title] = index
            self.sidebar_tab_scrolls[title] = page_scroll
        layout.addWidget(tabs, 1)

        # --- Text properties panel ---
        text_group = QGroupBox("Text Properties")
        self.text_controls_group = text_group
        ...
        self.background_controls_group = bg_group
        bg_lay = QVBoxLayout()
        ...
        self.flag_controls_group = flag_group
        ...
        self.logo_controls_group = logo_group
    ...
    # ------------------------------------------------------------------ selection
    def _show_sidebar_controls(self, tab_title, target_widget=None):
        """Open a sidebar tab and scroll the requested control group into view."""
        tabs = getattr(self, "sidebar_tabs", None)
        if tabs is None:
            return
        index = getattr(self, "sidebar_tab_indices", {}).get(tab_title)
        if index is None:
            return
        tabs.setCurrentIndex(index)

        scroll = getattr(self, "sidebar_tab_scrolls", {}).get(tab_title)
        if scroll is not None and target_widget is not None:
            # Run after the tab becomes visible so ensureWidgetVisible can use
            # the final viewport geometry.
            QTimer.singleShot(0, lambda s=scroll, w=target_widget:
                              s.ensureWidgetVisible(w, 18, 18))

    def _auto_open_controls_for_selection(self):
        """Route a clicked canvas element to its matching properties panel."""
        selected = self.scene.selectedItems()
        if not selected:
            return

        # A normal canvas click selects one item. For a multi-selection, only
        # auto-route when every selected item belongs to the same control type.
        if all(isinstance(item, LogoItem) for item in selected):
            self._show_sidebar_controls("Logo", getattr(self, "logo_controls_group", None))
            return
        if all(isinstance(item, CenteredTextItem) for item in selected):
            self._show_sidebar_controls("Text", getattr(self, "text_controls_group", None))
            return
        if all(type(item) is CenteredFlagItem for item in selected):
            self._show_sidebar_controls("Images", getattr(self, "flag_controls_group", None))
            return
        if len(selected) == 1 and selected[0] is self.bg_item:
            self._show_sidebar_controls("Images", getattr(self, "background_controls_group", None))
    ...
    def _on_selection_changed(self):
        ...
        # Canva-style behavior: clicking a design element immediately opens
        # the control panel that edits that exact element.
        self._auto_open_controls_for_selection()
