...
8. Player Image and Player Name have independent X/Y + arrow movement controls.
9. Player uploads stay at ORIGINAL source resolution. Player resizing is
...
class BandView(QGraphicsView):
    """Canvas interaction: select elements and directly drag player/name with the mouse."""

    def __init__(self, owner, scene):
        super().__init__(scene)
        self.owner = owner
        self._direct_drag_active = False

    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            scene_pos = self.mapToScene(event.position().toPoint())
            item = self.scene().itemAt(scene_pos, self.transform())
            target = self.owner._resolve_editable(item)
            self.owner.handle_canvas_click(item)

            # Player image and either name line can be dragged directly.
            # For a name drag, FIRST NAME + LAST NAME move together.
            if target in (
                self.owner.player_pix,
                self.owner.first_name_item,
                self.owner.last_name_item,
            ):
                self._direct_drag_active = self.owner.begin_direct_element_drag(target, scene_pos)
                if self._direct_drag_active:
                    event.accept()
                    return
    ...
    def mouseMoveEvent(self, event):
        if self._direct_drag_active:
            scene_pos = self.mapToScene(event.position().toPoint())
            self.owner.update_direct_element_drag(scene_pos)
            event.accept()
            return
        super().mouseMoveEvent(event)

    def mouseReleaseEvent(self, event):
        if self._direct_drag_active and event.button() == Qt.MouseButton.LeftButton:
            self.owner.end_direct_element_drag()
            self._direct_drag_active = False
            event.accept()
            return
...
class PlayerBandGenerator(QMainWindow):
    def __init__(self):
        ...
        self.statusBar().showMessage(
            "Drag Player Image or Player Name directly with the mouse. Drag other band areas to move the whole band."
        )
        self._fit_view()
    ...
    def _layout_player_name(self):
        """Move FIRST + LAST name together without changing their internal spacing."""
        x_offset = self.name_x_spin.value() if hasattr(self, "name_x_spin") else DEFAULTS["name_offset_x"]
        y_offset = self.name_y_spin.value() if hasattr(self, "name_y_spin") else DEFAULTS["name_offset_y"]
        fx, fy = BASE_POS["first_name"]
        lx, ly = BASE_POS["last_name"]
        self.first_name_item.setPos(fx + x_offset, fy + y_offset)
        self.last_name_item.setPos(lx + x_offset, ly + y_offset)

    def _nudge_spin(self, spin, delta):
        spin.setValue(spin.value() + delta)

    # ------------------------------------------------------------------ direct mouse dragging
    def begin_direct_element_drag(self, item, scene_pos):
        """Start mouse dragging for Player Image or the two-line Player Name."""
        if item not in (self.player_pix, self.first_name_item, self.last_name_item):
            return False

        self._begin_history_action()
        self._direct_drag_kind = "player" if item is self.player_pix else "name"
        self._direct_drag_start_local = self.band.mapFromScene(scene_pos)

        if self._direct_drag_kind == "player":
            self._direct_drag_start_x = self.player_x_spin.value()
            self._direct_drag_start_y = self.player_y_spin.value()
            self.statusBar().showMessage(
                "Dragging Player Image — release mouse to place it.", 2500
            )
        else:
            self._direct_drag_start_x = self.name_x_spin.value()
            self._direct_drag_start_y = self.name_y_spin.value()
            self.statusBar().showMessage(
                "Dragging Player Name — FIRST + LAST move together.", 2500
            )
        return True

    def update_direct_element_drag(self, scene_pos):
        """Move the active element using mouse movement converted into band-local pixels."""
        kind = getattr(self, "_direct_drag_kind", None)
        start_local = getattr(self, "_direct_drag_start_local", None)
        if kind is None or start_local is None:
            return

        current_local = self.band.mapFromScene(scene_pos)
        dx = current_local.x() - start_local.x()
        dy = current_local.y() - start_local.y()
        new_x = round(self._direct_drag_start_x + dx)
        new_y = round(self._direct_drag_start_y + dy)

        if kind == "player":
            # Block one signal at a time so layout runs only after both values are ready.
            self.player_x_spin.blockSignals(True)
            self.player_y_spin.blockSignals(True)
            self.player_x_spin.setValue(new_x)
            self.player_y_spin.setValue(new_y)
            self.player_x_spin.blockSignals(False)
            self.player_y_spin.blockSignals(False)
            self._layout_player_image()
        else:
            self.name_x_spin.blockSignals(True)
            self.name_y_spin.blockSignals(True)
            self.name_x_spin.setValue(new_x)
            self.name_y_spin.setValue(new_y)
            self.name_x_spin.blockSignals(False)
            self.name_y_spin.blockSignals(False)
            self._layout_player_name()

    def end_direct_element_drag(self):
        """Finish a direct mouse drag as one clean undo/redo history action."""
        self._commit_history_now()
        self._direct_drag_kind = None
        self._direct_drag_start_local = None
        self.statusBar().showMessage(
            "Placed. Drag the player image or player name again anytime.", 2500
        )
    ...
    def _build_sidebar(self):
        ...
        image_layout.addWidget(QLabel("Position — drag image on canvas with mouse, or use X / Y"))
        offsets = QHBoxLayout()
        self.player_x_spin = self._make_spin(-1000, 1000, DEFAULTS["player_offset_x"], prefix="X ")
        self.player_y_spin = self._make_spin(-1000, 1000, DEFAULTS["player_offset_y"], prefix="Y ")
        ...
        image_nudge = QHBoxLayout()
        img_left = QPushButton("←")
        img_up = QPushButton("↑")
        img_down = QPushButton("↓")
        img_right = QPushButton("→")
        for btn in (img_left, img_up, img_down, img_right):
            btn.setObjectName("nudgeButton")
        img_left.clicked.connect(lambda: self._nudge_spin(self.player_x_spin, -5))
        img_right.clicked.connect(lambda: self._nudge_spin(self.player_x_spin, 5))
        img_up.clicked.connect(lambda: self._nudge_spin(self.player_y_spin, -5))
        img_down.clicked.connect(lambda: self._nudge_spin(self.player_y_spin, 5))
        image_nudge.addWidget(img_left)
        image_nudge.addWidget(img_up)
        image_nudge.addWidget(img_down)
        image_nudge.addWidget(img_right)
        image_layout.addLayout(image_nudge)
        data_root.addWidget(image_group)

        name_group = QGroupBox("Player Name Position")
        name_layout = QVBoxLayout(name_group)
        name_layout.addWidget(QLabel("Drag either name line on canvas — FIRST + LAST move together"))
        name_offsets = QHBoxLayout()
        self.name_x_spin = self._make_spin(-1000, 1000, DEFAULTS["name_offset_x"], prefix="X ")
        self.name_y_spin = self._make_spin(-1000, 1000, DEFAULTS["name_offset_y"], prefix="Y ")
        self.name_x_spin.valueChanged.connect(lambda _: self._layout_player_name())
        self.name_y_spin.valueChanged.connect(lambda _: self._layout_player_name())
        name_offsets.addWidget(self.name_x_spin)
        name_offsets.addWidget(self.name_y_spin)
        name_layout.addLayout(name_offsets)

        name_nudge = QHBoxLayout()
        name_left = QPushButton("←")
        name_up = QPushButton("↑")
        name_down = QPushButton("↓")
        name_right = QPushButton("→")
        for btn in (name_left, name_up, name_down, name_right):
            btn.setObjectName("nudgeButton")
        name_left.clicked.connect(lambda: self._nudge_spin(self.name_x_spin, -5))
        name_right.clicked.connect(lambda: self._nudge_spin(self.name_x_spin, 5))
        name_up.clicked.connect(lambda: self._nudge_spin(self.name_y_spin, -5))
        name_down.clicked.connect(lambda: self._nudge_spin(self.name_y_spin, 5))
        name_nudge.addWidget(name_left)
        name_nudge.addWidget(name_up)
        name_nudge.addWidget(name_down)
        name_nudge.addWidget(name_right)
        name_layout.addLayout(name_nudge)
        data_root.addWidget(name_group)
    ...
    def _reset_defaults(self):
        ...
        self.name_x_spin.setValue(DEFAULTS["name_offset_x"])
        self.name_y_spin.setValue(DEFAULTS["name_offset_y"])
    ...
    def _capture_state(self):
        ...
        return {
            ...
            "name_x": self.name_x_spin.value(),
            "name_y": self.name_y_spin.value(),
    ...
    def _apply_state(self, state):
        ...
        self.name_x_spin.setValue(int(state.get("name_x", DEFAULTS["name_offset_x"])))
        self.name_y_spin.setValue(int(state.get("name_y", DEFAULTS["name_offset_y"])))
