...
class UpscalerGUI:
    ...
    def _setup_ui(self):
        ...
        # Face enhance — explicitly OFF at every launch
        self.face_enhance.set(False)
        self.face_enhance_button = self.create_color_toggle(
            step2_frame,
            "AI Face Enhance",
            self.face_enhance
        )
        self.face_enhance_button.pack(anchor="w", pady=(0, 10))
        self.face_enhance_button.configure(
            text="✕ OFF AI Face Enhance",
            bg="#B22222",
            fg="white",
            activebackground="#CD5C5C"
        )
        ...
        self.benchmark_button = ttk.Button(
            button_row,
            text="⚙ Benchmark 5m / 5MB",
            command=self.start_benchmark_thread,
            style="Secondary.TButton",
        )
        self.benchmark_button.pack(side="left", padx=(0, 8))
    ...
    def get_expected_output_path(self, image_path, output_dir):
        """Return the exact output path for the current settings without processing the image."""
    ...
    def scan_completed_outputs(self):
        """Remove completed images from the pending queue and return how many already exist."""
        completed = 0
        remaining = deque()
        with self.queue_lock:
            while self.pending_images:
                image_path, source_path = self.pending_images.popleft()
                output_dir = self.get_output_path(source_path)
                output_path = self.get_expected_output_path(image_path, output_dir)
                if output_path.exists() and output_path.is_file() and output_path.stat().st_size > 0:
                    completed += 1
                else:
                    remaining.append((image_path, source_path))
            self.pending_images.extend(remaining)
        return completed
    ...
    def upscale_one_image(self, image_path, output_dir, upsampler, face_enhancer):
        ...
        # Check before expensive AI inference so reruns resume instantly.
        output_path = self.get_expected_output_path(image_path, output_dir)
        if output_path.exists() and output_path.is_file() and output_path.stat().st_size > 0:
    ...
    def _get_benchmark_image(self):
        """Return the first supported queued image without changing the live queue."""
        for source in list(self.input_paths):
            try:
                images = self.get_images_from_path(source)
            except Exception:
                continue
            if images:
    ...
    def start_benchmark_thread(self):
        if self.running:
            ...
            return
        image_path = self._get_benchmark_image()
        ...
        thread = threading.Thread(target=self.run_benchmark, args=(image_path,), daemon=True)
        thread.start()

    def _encode_candidate(self, image, fmt, quality):
        if fmt == "WEBP":
            ok, encoded = cv2.imencode(".webp", image, [cv2.IMWRITE_WEBP_QUALITY, int(quality)])
        elif fmt == "JPG":
        ...
        if not ok:
            raise RuntimeError(f"Could not encode benchmark image as {fmt}.")
        return encoded

    def run_benchmark(self, image_path):
        """Benchmark current AI settings and find the highest quality output under 5 MB."""
        benchmark_start = time.perf_counter()
        try:
            self.log("")
            self.log(f"⚙ Benchmarking: {image_path.name}")
            self.log(f"Model: {self.model_name.get()} • Scale: {self.outscale.get():g}x")
            self.log("This uses a temporary output and will not overwrite processed images.")

            img = cv2.imread(str(image_path), cv2.IMREAD_UNCHANGED)
            if img is None:
                raise RuntimeError("Could not read the benchmark image.")
            ...
            # Search highest practical quality. WEBP is preferred at equal quality
            # because it usually provides the smallest Leonardo-compatible output.
            candidates = []
            encode_start = time.perf_counter()
            for quality in range(100, 49, -2):
                for fmt in ("WEBP", "JPG"):
                    encoded = self._encode_candidate(output, fmt, quality)
                    size_bytes = len(encoded)
                    if size_bytes <= LEONARDO_MAX_SIZE_BYTES:
                        candidates.append((quality, fmt, size_bytes, encoded))
                if candidates:
                    break

            # PNG is tested as a lossless bonus candidate, but only selected when it fits.
            png_encoded = self._encode_candidate(output, "PNG", 100)
            if len(png_encoded) <= LEONARDO_MAX_SIZE_BYTES:
                candidates.append((101, "PNG", len(png_encoded), png_encoded))

            encode_seconds = time.perf_counter() - encode_start
            total_seconds = time.perf_counter() - benchmark_start

            if candidates:
                # Highest quality first; at equal quality prefer WEBP, then JPG.
                preference = {"PNG": 3, "WEBP": 2, "JPG": 1}
                quality, fmt, size_bytes, encoded = max(
                    candidates, key=lambda item: (item[0], preference[item[1]])
                )
                size_mb = size_bytes / 1024 / 1024
                under_time = total_seconds <= 300
                under_size = size_bytes <= LEONARDO_MAX_SIZE_BYTES
                passed = under_time and under_size

                display_quality = "Lossless" if fmt == "PNG" else f"{quality}%"
                self.log(f"Benchmark AI time: {inference_seconds:.1f}s")
                self.log(f"Model loading time: {model_load_seconds:.1f}s")
                self.log(f"Best ≤5MB result: {fmt} at {display_quality} • {size_mb:.2f}MB")
                self.log(f"Total benchmark time: {total_seconds:.1f}s")
                self.log("✅ PASS: Best tested quality fits under 5 minutes and 5MB." if passed else
                         "⚠ FAIL: The tested image does not meet both limits with current AI settings.")

                if fmt == "WEBP":
                    self.output_format.set("WEBP (smaller)")
                    self.output_quality.set(int(quality))
                elif fmt == "JPG":
                    self.output_format.set("JPG (standard)")
                    self.output_quality.set(int(quality))
                else:
                    self.output_format.set("PNG (lossless)")

                verdict = (
                    f"PASS\n\nRecommended: {fmt} at {display_quality}\n"
                    f"Estimated output: {size_mb:.2f} MB\n"
                    f"Actual total time: {total_seconds:.1f} seconds\n\n"
                    "The recommended format and quality have been applied."
                    if passed else
                    f"LIMIT NOT MET\n\nBest size found: {fmt} at {display_quality} ({size_mb:.2f} MB)\n"
                    f"Actual total time: {total_seconds:.1f} seconds\n\n"
                    "Try Fast 2x, reduce scale, disable Face Enhance, or use a GPU."
                )
            else:
                verdict = (
                    f"No WEBP/JPG result reached 5 MB at quality 50 or higher.\n\n"
                    f"Actual AI time: {inference_seconds:.1f} seconds.\n"
                    "Use a smaller scale or allow stronger resizing/compression."
                )
                self.log("⚠ No output reached 5MB at quality 50 or higher.")

            messagebox.showinfo("5 Minute / 5MB Benchmark", verdict)
    ...
    def run_upscale(self):
        try:
            ...
            # Resume from existing outputs: completed files count toward progress
            # and are removed from the active pending queue before model loading.
            already_completed = self.scan_completed_outputs()
            self.processed_images = already_completed
            ...
            if already_completed:
                self.log(f"↩ Resume detected: {already_completed}/{self.total_images} image(s) already completed.")
            ...
            with self.queue_lock:
                has_pending = bool(self.pending_images)
            if not has_pending:
                self.log("✅ All selected images are already processed. Nothing new to enhance.")
                return
