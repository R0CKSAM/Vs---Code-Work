...
class UpscalerGUI:
    def __init__(self, root):
        ...
        self.input_paths = []  # List of folders or files
    ...
    def log(self, message):
        ...
        # Safe vertical expansion when logs grow
        try:
            lines = int(self.log_text.index("end-1c").split(".")[0])
            if lines > 12:
                current = self.root.winfo_height()
                self.root.geometry(
                    f"{self.root.winfo_width()}x{min(current + 40, 950)}"
                )
        except Exception:
            pass
    ...
    def remove_selected_item(self):
        try:
            selected_index = self.selection_text.curselection()
            if not selected_index:
                messagebox.showinfo("Remove", "Select an item from the list first.")
                return

            selected = self.selection_text.get(selected_index[0])

            self.input_paths = [
                p for p in self.input_paths
                if Path(p).name not in selected
            ]
    ...
    def select_multiple_folders(self):
        ...
        if folders:
            self.input_paths.extend(folders)
            self.update_selection_display()
    ...
    def run_upscale(self):
        try:
            ...
            all_images = []
            folder_mapping = {}  # Map image to its source folder for output

            for input_path in self.input_paths:
                images = self.get_images_from_path(input_path)
                for img in images:
                    all_images.append(img)
                    folder_mapping[str(img)] = input_path
            ...
            self.log("")
            self.log(f"🚀 Starting to upscale {self.total_images} image(s)...")
            ...
            for image_path in all_images:
                ...
                # Update progress
                progress_pct = int((self.processed_images / self.total_images) * 100)
                self.progress["value"] = self.processed_images
                self.progress_label.config(text=f"{progress_pct}%")

                # Calculate and display time remaining
                elapsed = time.time() - self.start_time
                remaining = self.estimate_time(self.processed_images, self.total_images, elapsed)
                ...
                try:
                    saved_path, file_size = self.upscale_one_image(
            ...
            if not self.stop_requested:
                self.log(f"✅ All done! Processed {self.processed_images} images in {str(timedelta(seconds=int(elapsed)))}")
            
            self.time_label.config(text="")
            messagebox.showinfo("Done!", f"✅ Upscaled {self.processed_images} image(s)!\n\nAll files saved to 'upscaled' folders.")
...
def add_queue_files(self):
    ...
    if files:
        self.processing_queue.extend(files)
...
def add_queue_folder(self):
    ...
    if folder:
        self.processing_queue.append(folder)
        self.update_queue_view()
