...
window.RevenueCharts=(()=>{
  const palette=['#0B6B54','#A9781F','#2E6E8E','#8A5A9C','#B0562F','#4D7A5A','#5B6B9E','#8A6A4A'];
  ...
  const emptyPlugin={id:'zeroNotice',afterDraw(chart){if(['doughnut','pie'].includes(chart.config.type)&&chart.data.datasets[0].data.every(v=>v===0)){const{ctx,chartArea:a}=chart;ctx.save();ctx.fillStyle='#5C6D67';ctx.font='14px Arial';ctx.textAlign='center';ctx.fillText('No revenue in this selection',(a.left+a.right)/2,(a.top+a.bottom)/2);ctx.restore();}}};
  ...
  function base(){return{
    ...
    plugins:{legend:{display:false,labels:{boxWidth:10,font:{size:11},color:'#5C6D67'}}},
    scales:{
      x:{grid:{display:false},ticks:{color:'#5C6D67',maxRotation:0,autoSkip:true,maxTicksLimit:8}},
      y:{beginAtZero:true,border:{display:false},grid:{color:'#EAEEEB'},ticks:{color:'#5C6D67',callback:short}}
    }
  };}
  ...
  function render(values=rows){
    ...
    draw('trendChart','line',{labels,datasets:[{label:metric,data:a.dates.map(([,v])=>v[metric]/divisor),borderColor:palette[0],backgroundColor:'#0B6B5414',fill:true,tension:.15,pointRadius:labels.length>40?0:3,pointHoverRadius:6,borderWidth:2}]},options);
    ...
    options={responsive:true,maintainAspectRatio:false,animation:false,cutout:'65%',plugins:{
      legend:{position:'right',labels:{boxWidth:10,font:{size:11},color:'#16241F',generateLabels:chart=>chart.data.labels.map((text,i)=>({text:text.length>24?text.slice(0,23)+'...':text,fillStyle:chart.data.datasets[0].backgroundColor[i],strokeStyle:'transparent',index:i}))}},
    ...
    draw('shareChart',shareType,{labels:top.map(([name])=>name),datasets:[{data:revenue,backgroundColor:palette,borderWidth:2,borderColor:'#F6F7F4'}]},options);
    ...
    options=base();options.indexAxis='y';options.scales.x={beginAtZero:true,grid:{color:'#EAEEEB'},title:{display:true,text:'INR'},ticks:{callback:short}};options.scales.y={grid:{display:false},ticks:{font:{size:11},callback:function(v){const label=this.getLabelForValue(v);return label.length>23?label.slice(0,22)+'...':label;}}};options.plugins.tooltip={callbacks:{label:c=>rupee(c.parsed.x)}};
