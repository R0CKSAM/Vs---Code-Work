...
<main>
  ...
  <section class="networks-section" id="networksSection" aria-label="Networks and ASNs">
    <div class="device-os-header"><h2>Networks &amp; ASNs</h2><span class="device-os-status" id="networkStatus"></span></div>
    <div class="network-book">
      ...
      <article class="device-os-panel"><header><span>ASN Network Ranking</span><span class="device-os-count" id="networkCount"></span></header><div class="device-os-list network-list" id="networkList"></div></article>
      <div class="network-footer"><span id="networkWatchHours">Watch Hours: 0</span><span>IP activity is summed by day</span></div>
    </div>
  </section>
  </div>
</main>
...
function renderNetworks(channelActive,regionActive,stateActive,platformActive){{
  const status=byId('networkStatus'),list=byId('networkList'),count=byId('networkCount'),coverage=byId('networkCoverage'),counts=byId('networkCounts'),watch=byId('networkWatchHours');
  const unavailable=channelActive||regionActive||stateActive||(state.source==='fast'&&platformActive);
  if(unavailable){{
    status.textContent='Available for Date and Source filters';count.textContent='';coverage.textContent='Decoded coverage: Not available';counts.textContent='Source-level mart';watch.textContent='Watch Hours: Not available';
  ...
  status.textContent='All-status source network evidence';
  const totals=new Map();
  asnRows.forEach(row=>{{
    if(row[1]!==state.source||(!state.from||row[0]>=state.from)===false||(!state.to||row[0]<=state.to)===false)return;
  ...
  const max=ranked.length?ranked[0].rawRows:0,totalHours=allRows/600;
  coverage.textContent=`Decoded coverage: ${{allRows?(decodedRows*100/allRows).toFixed(1):'0.0'}}%`;counts.textContent=`${{formatNumber.format(decoded.length)}} decoded | ${{formatNumber.format(all.length-decoded.length)}} unresolved`;count.textContent=`${{formatNumber.format(ranked.length)}} of ${{formatNumber.format(all.length)}} ASNs`;watch.textContent=`Watch Hours: ${{formatHours.format(totalHours)}}`;
  list.innerHTML=ranked.length?ranked.map((row,index)=>{{const hours=row.rawRows/600,width=max?Math.max(1,row.rawRows*100/max):0,name=networkIsDecoded(row)?row.name:'Unknown / NA',metadata=[row.country,row.type,row.domain].filter(value=>value&&value!=='Unknown / NA').join(' | ')||'Decoder mapping unavailable';return `<div class="network-row"><span class="ua-rank">${{index+1}}</span><div class="network-main"><div class="network-name" title="AS${{escapeHtml(row.asn)}} - ${{escapeHtml(name)}}">AS${{escapeHtml(row.asn)}} - ${{escapeHtml(name)}}</div><div class="network-meta" title="${{escapeHtml(metadata)}}">${{escapeHtml(metadata)}}</div><div class="ua-track"><div class="ua-fill" style="width:${{width.toFixed(2)}}%"></div></div></div><div class="network-value">${{escapeHtml(formatHours.format(hours))}}<span>${{escapeHtml(formatNumber.format(row.ips))}} IP-day activity</span></div></div>`}}).join(''):'<div class="ua-empty">No matching ASN data</div>';
}}
