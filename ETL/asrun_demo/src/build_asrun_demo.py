...
def build_payload(
    ...
    return {
        ...
        "kpis": {
            ...
            "skipped_ad_events": int(
                ads["status"].fillna("").astype("string").str.strip().str.casefold().eq("skipped").sum()
            ),
        ...
        "events": records(
            ...
            ["on_air_start_ist", "on_air_end_ist", "ad_type", "event_id", "creative_title",
             "actual_duration_seconds", "status", "source_file", "source_line", "brand", "campaign"],
        ),
...
function asrunEventStatus(event){return String(event.status||'').trim().toUpperCase()}
function isSkippedAsrunEvent(event){return asrunEventStatus(event)==='SKIPPED'}
function isMissedAsrunEvent(event){return asrunEventStatus(event)==='MISSED'}
function asrunStatusSuffix(event){const status=asrunEventStatus(event);return status==='SKIPPED'||status==='MISSED'?' | '+status:''}
function render(){const ev=filtered(),seconds=ev.reduce((n,e)=>n+(+e.actual_duration_seconds||0),0),grouped=new Map();for(const e of ev){const k=e.ad_type+'\u0000'+e.event_id+'\u0000'+e.creative_title,g=grouped.get(k)||{type:e.ad_type,id:e.event_id,title:e.creative_title,plays:0,seconds:0};g.plays++;g.seconds+=+e.actual_duration_seconds||0;grouped.set(k,g);}const rankings=[...grouped.values()].sort((a,b)=>b.seconds-a.seconds),spot=rankings.filter(x=>x.type==='Spot'),lband=rankings.filter(x=>x.type==='L-band'),spotPlays=ev.filter(x=>x.ad_type==='Spot').length,lbandPlays=ev.filter(x=>x.ad_type==='L-band').length,skipped=ev.filter(isSkippedAsrunEvent).length,missed=ev.filter(isMissedAsrunEvent).length,other=ev.length-skipped-missed,statusNote=fmt(other)+' without non-delivery status | '+fmt(skipped)+' SKIPPED | '+fmt(missed)+' MISSED',cards=[{label:'Total ASRUN ad events',value:fmt(ev.length),note:statusNote},{label:'Total actual ad duration',value:mins(seconds),note:'Sum of classified ASRUN source durations'},{label:'Total unique creatives',value:fmt(rankings.length),note:'Distinct Ad ID + creative title combinations'},{label:'Spot ASRUN events',value:fmt(spotPlays)+' events',note:fmt(spot.length)+' unique Spot creatives'},{label:'L-band ASRUN events',value:fmt(lbandPlays)+' events',note:fmt(lband.length)+' unique L-band creatives'}];$('kpis').innerHTML=cards.map(c=>'<div class="card"><div class="label">'+c.label+'</div><div class="value">'+c.value+'</div><div class="card-note">'+c.note+'</div></div>').join('');rankingBars($('spotBars'),spot);rankingBars($('lbandBars'),lband);renderAudience(ev);}
...
function audienceLines(events,state){const preview=events.slice().sort((a,b)=>String(a.on_air_start_ist).localeCompare(String(b.on_air_start_ist))).slice(-50).reverse();if(!preview.length)return '<div class="audience-empty">No ASRUN ad events in this selection.</div>';return preview.map(event=>{const metric=audienceValue(event,state),status=asrunStatusSuffix(event);return '<div class="event-line"><span>'+formatIst(event.on_air_start_ist)+'</span><span><strong>'+esc(event.event_id)+'</strong><small>'+esc(event.ad_type+status)+'</small></span><span>'+esc(event.creative_title)+'</span><span class="duration">'+fmt(event.actual_duration_seconds)+' sec</span><span class="audience-value">'+esc(metric.value)+'</span></div>';}).join('');}
...
function combinedRows(events,fast,stream){const amagi=amagiMinuteMap();return events.sort((a,b)=>a.on_air_start_ist.localeCompare(b.on_air_start_ist)).map(e=>{const fastMetric=audienceValue(e,fast),streamMetric=audienceValue(e,stream),amagiMetric=audienceValue(e,amagi),youtubeMetric=youtubeFiveMinuteValue(e),metrics={FAST:fastMetric,STREAM:streamMetric,AMAGI:amagiMetric,YouTube:youtubeMetric},missingSources=Object.entries(metrics).filter(([_name,metric])=>metric.total===null).map(([name])=>name),all=Object.values(metrics).map(metric=>metric.total);return {event:e,fast:fastMetric,stream:streamMetric,amagi:amagiMetric,youtube:youtubeMetric,missingSources,partial:missingSources.length>0,total:missingSources.length?null:all.reduce((sum,v)=>sum+v,0)};});}
function deliveryCoverageStatus(row){return row.missingSources.length?'Missing source data: '+row.missingSources.join(', '):'All sources covered';}
function combinedLines(events,fast,stream){const rows=combinedRows(events,fast,stream),preview=rows.slice(-50).reverse();if(!preview.length)return '<div class="audience-empty">No ASRUN ad events in this selection.</div>';return preview.map(row=>{const e=row.event,total=row.total===null?'No combined data':fmt(row.total),status=asrunStatusSuffix(e);return '<div class="combined-line"><span>'+formatIst(e.on_air_start_ist)+'</span><span><strong>'+esc(e.event_id)+'</strong><small>'+esc(e.ad_type+status)+'</small></span><span>'+esc(e.creative_title)+'</span><span class="duration">'+fmt(e.actual_duration_seconds)+' sec</span><span class="combined-value fast-col">'+esc(row.fast.value)+'</span><span class="combined-value stream-col">'+esc(row.stream.value)+'</span><span class="combined-value amagi-col">'+esc(row.amagi.value)+'</span><span class="combined-value youtube-col">'+esc(row.youtube.value)+'</span><span class="combined-value total-col">'+esc(total)+'</span></div>';}).join('');}
function renderAudience(events){const range=masterDashboardRange();if(!dashboardSourceRangeLoaded('viewer',range.start,range.end)||!dashboardSourceRangeLoaded('amagi',range.start,range.end)){const failed=dashboardSourceError('viewer')||dashboardSourceError('amagi'),message=failed||'Loading FAST, STREAM, and AMAGI audience data...';$('allRows').innerHTML='<div class="audience-empty">'+esc(message)+'</div>';$('allNote').textContent='';return}refreshAmagiFilters();const fast=audienceMinuteMap('fast'),stream=audienceMinuteMap('stream'),visible=Math.min(events.length,50),note='Showing latest '+fmt(visible)+' of '+fmt(events.length)+' ASRUN ad events. CSV exports the complete filtered result.';$('allRows').innerHTML=combinedLines(events,fast,stream);$('allNote').textContent=note;}
...
function exportAllEventsCsv(){const events=filtered(),fast=audienceMinuteMap('fast'),stream=audienceMinuteMap('stream'),amagi=amagiMinuteMap(),filters=exportFilterContext(),fastPlatforms=exportSelection('fastPlatform','All FAST platforms',true),fastChannels=exportSelection('fastChannel','All FAST channels',true),streamChannels=exportSelection('streamChannel','All STREAM channels',true),amagiPlatforms=exportSelection('amagiPlatform','All AMAGI platforms',true),amagiChannels=exportSelection('amagiChannel','All AMAGI channels',true),youtubeChannels=indiaTvYoutubeScopeLabel(),metricBasis='FAST / STREAM / AMAGI: sum of five one-minute concurrency samples; YouTube: exact event minute',header=['Selected Date From','Selected Date To','Selected Ad Types','Selected Ad IDs','Selected Creative Titles','On-air IST','ASRUN Status','ASRUN Source File','ASRUN Source Line','Ad Type','Ad ID','Creative Title','Actual Duration Seconds','5-Minute Window IST','FAST Platforms','FAST Channels','STREAM Channels','AMAGI Platforms','AMAGI Channels','India TV YouTube Channel','FAST 5-Minute Concurrency','STREAM 5-Minute Concurrency','AMAGI 5-Minute Actual Concurrency','India TV YouTube Scope','India TV YouTube Minute Concurrency','India TV YouTube Active Live Videos','India TV YouTube Active Video IDs','India TV YouTube Active Video Titles','Combined Concurrency','Coverage Status','Metric Basis'],rows=combinedRows(events,fast,stream).map(row=>[filters.dateFrom,filters.dateTo,filters.adTypes,filters.adIds,filters.creatives,formatIstSeconds(row.event.on_air_start_ist),row.event.status||'',row.event.source_file||'',row.event.source_line??'',row.event.ad_type,row.event.event_id,row.event.creative_title,row.event.actual_duration_seconds,row.fast.window,fastPlatforms,fastChannels,streamChannels,amagiPlatforms,amagiChannels,youtubeChannels,row.fast.total,row.stream.total,row.amagi.total,row.youtube.scope,row.youtube.total,row.youtube.live_videos,row.youtube.video_ids,row.youtube.video_titles,row.total??'',deliveryCoverageStatus(row),metricBasis]);downloadCsv('asrun_all_delivered_events_'+deliveryAdTypeFileToken()+'_'+filters.dateFrom+'_to_'+filters.dateTo+'.csv',header,rows);}
...
function exportAudienceBreakdownCsv(){const events=filtered(),filters=exportFilterContext(),scopes=[...audienceBreakdownScopes('fast'),...audienceBreakdownScopes('stream'),...amagiBreakdownScopes()],header=['Selected Date From','Selected Date To','Selected Ad Types','Selected Ad IDs','Selected Creative Titles','On-air IST','ASRUN Status','ASRUN Source File','ASRUN Source Line','Ad Type','Ad ID','Creative Title','Actual Duration Seconds','Source','Platform','Channel','5-Minute Window IST','Individual 5-Minute Concurrency','Metric Basis'],rows=[];for(const scope of scopes){const map=scope.source==='AMAGI'?amagiScopeMap(scope):audienceScopeMap(scope),basis=scope.source==='AMAGI'?'Actual platform-reported concurrent viewers':'Unique IP minute sum';for(const event of events){const metric=audienceScopeValue(event,map);rows.push([filters.dateFrom,filters.dateTo,filters.adTypes,filters.adIds,filters.creatives,formatIstSeconds(event.on_air_start_ist),event.status||'',event.source_file||'',event.source_line??'',event.ad_type,event.event_id,event.creative_title,event.actual_duration_seconds,scope.source,scope.platform,scope.channel,metric.window,metric.total,basis])}}downloadCsv('asrun_audience_platform_channel_breakdown_'+deliveryAdTypeFileToken()+'_'+filters.dateFrom+'_to_'+filters.dateTo+'.csv',header,rows)}
...
function initializeYoutubeDates(){
  const bounds=youtubeTrueBounds();
  // YouTube collection can be newer than the latest ASRUN file. Start on its
  // latest available day so current concurrency is visible immediately;
  // Follow Main remains available when users need ad-event correlation.
...
youtubeDeliveryDetails=function(event){
  ...
  if(!dashboardSourceRangeLoaded('youtube',eventDate,eventDate)){
    ...
    return {
    value:outsideSelection?'N/A':dashboardSourceError('youtube')||'Loading',
    total:null,
    live_videos:0,
    video_ids:'',
    video_titles:'',
  ...
  if(!channels.size)return {
    value:'N/A',total:null,live_videos:0,video_ids:'',video_titles:'',
  ...
  if(!youtubeDeliveryMinuteIndex.has(selectionKey)){
    ...
    for(const row of youtube.collector_minute||[]){
      if(!channels.has(String(row.youtube_channel||'Unknown / NA')))continue;
      const minuteKey=youtubeMinuteKey(row.timestamp_ist);
      collectorDates.add(isoDatePart(row.timestamp_ist));
    ...
    youtubeDeliveryMinuteIndex.set(
      selectionKey,{totals,videos,collectorStatuses,collectorDates}
    );
  }
  ...
  if(!hasMinute){
    ...
    return {
      ...
      return index.collectorDates.has(eventDate)
        ?'Missing India TV YouTube collector minute'
        :'No India TV YouTube collection for date';
    })(),
    };
  }
...
function refreshNctStorySourceFilters(){
  ...
  for(const row of ((DATA.youtube||{}).video_daily||[])){
    ...
    if(id&&String(row.title||'').trim()){
      titleById.set(id,channel+' | '+String(row.title).trim());
    }
  }
  buildMulti(
...
function refreshNctFilters(){
  ...
  for(const [id,key,kind] of NCT_FILTER_SPECS){
    ...
    const existing=$(id+'Menu').querySelectorAll('input[data-value]').length;
    if(values.length&&!existing){
      // An empty no-overlap date universe is not an intentional user clear.
      multiInitialized.delete(id);
    }
...
const asrunBaseBuildMulti=buildMulti;
buildMulti=function(id,items,kind,defaultValues,onChange){
  const menu=$(id+'Menu');
  const existing=[...menu.querySelectorAll('input[data-value]')];
  const selected=selectedMulti(id);
  if(existing.length&&selected.size===existing.length){
    menu.innerHTML='';
    multiInitialized.delete(id);
  }
  return asrunBaseBuildMulti(id,items,kind,defaultValues,onChange);
};
...
function audienceValue(event,state){
  ...
  if(!coverage.has(window.keys[0])){
    return {value:'N/A',window:window.label,total:null,available:false};
  }
  let total=0;
