...
$rawPath = Join-Path $LakeRoot "year=$year\month=$month\day=$day\part_stream_$($Date.ToString('yyyy_MM_dd'))_0.parquet"
...
& $Python (Join-Path $PSScriptRoot 'src\tools\export_vod_query_events.py') --input $rawPath --date $dateKey --out $dailyCsv
