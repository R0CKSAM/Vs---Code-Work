...
def build_cmcd_table(con, args: argparse.Namespace) -> None:
    ...
    con.execute(
        ...
        SELECT
            ...
            approx_count_distinct(cmcd_sid)::BIGINT AS distinct_cmcd_sessions,
            approx_count_distinct(cmcd_cid)::BIGINT AS distinct_cmcd_content_ids,
            ...
            approx_count_distinct(UA)::BIGINT AS distinct_uas,
            approx_count_distinct(COALESCE(cliIP, '') || '|' || COALESCE(UA, ''))::BIGINT AS distinct_ipua_pairs
        FROM resolved
...
def write_manifest(args: argparse.Namespace, new_rows: int, output_path: Path) -> None:
    manifest = {
        ...
        "metric_notes": {
            "distinct_cmcd_sessions": "Approximate daily distinct CMCD sid values on FAST .ts rows, using DuckDB approx_count_distinct to keep large-day ETL memory-safe. This is playback-session telemetry, not app queryStr session_id.",
            "distinct_cmcd_content_ids": "Approximate daily distinct CMCD cid values on FAST .ts rows, using DuckDB approx_count_distinct to keep large-day ETL memory-safe.",
